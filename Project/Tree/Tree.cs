...
namespace TreeStructure
{
    ...
    public class Tree
    {
        ...
        public bool TryFind(int value, out Tree tree)
        {
            if (Value == value)
            {
                tree = this; return true;
            }

            if (Value > value && LeftBranch != null)
            {
                return LeftBranch.TryFind(value, out tree);
            }
            else if (Value < value && RightBranch != null)
            {
                return RightBranch.TryFind(value, out tree);
            }
            else
            {
        ...
        public static Tree GenTree()
        {
            ...
            for (int i = 0; i < new Random().Next(0, 100); i++)
                tree.BinaryEnter(new Random().Next(0,100));
